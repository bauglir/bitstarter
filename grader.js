...
var downloadFromUrl = function(url, checks) {
  var downloadedFile = 'downloaded.html';

  restler.get(url).on('complete', function(result, response) {
    fs.writeFileSync(downloadedFile, result);
    verifyFile(downloadedFile, checks);
  });
};
...
var verifyFile = function(fileToCheck) {
...
if(require.main == module) {
  program
    ...
    .option('-u, --url <url>', 'URL to remote site')
    .parse(process.argv);
  ...
  if(typeof program.url !== 'undefined') {
    ...
    verifyFile(program.file, program.checks);
  }
